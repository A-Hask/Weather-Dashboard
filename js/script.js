...
var weatherInfo = function (weatherData, city) {
    ...
    currentWeather.append("<li>The weather in " + city + " today (" + moment().format("MMM DD, YYYY") + "): " 
        ...
        + "</span>.</li>");
    forecast.empty();
    forecast.append("<h3>" + city + "</h3><br>");
    for (var i = 1; i < 6; i++) {
        forecast.append("<li class='col-12 row'>On " + moment().add(i, 'days').format("MMM DD, YYYY") + ", there will be " + weatherData.daily[i].weather[0].description + "."
...
function search() {
    ...
    //future feature: empty search bar
